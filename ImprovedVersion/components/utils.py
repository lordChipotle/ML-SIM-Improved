...
def get_death_rate(mortality, recovery,icu, ob_period=25, cure_period=10):
    death = 0
    cur = 1
    lambda1 = 0.25
    for i in range(ob_period):
        new_death = (cur/icu) * mortality
        death += cur * mortality
        cur -= new_death
        if i >= cure_period-1:
...
def make_fake_data():
    ...
    keys = ['dead', 'cured', 'observed', 'cum_cured', 'cum_dead', 'cum_confirmed','icuover_mean']
    res = {}
    for item in cities:
